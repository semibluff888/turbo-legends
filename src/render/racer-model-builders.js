...
import { AVATARS_BY_ID } from '../game/appearance.js';
...
export function disposeGroup(group) {
  if (!group) return;
  group.traverse((obj) => {
    if (obj.geometry && !obj.geometry.userData?.sharedRuntimeAsset) obj.geometry.dispose();
    if (obj.material) {
...
export function buildCyberHypercar(primaryColor = 0x0f172a, accentColor = 0x00f0ff, avatarId = 'cat', showDriver = true, nitroBoost = false) {
  ...
  driverMount.position.set(0, 0.42, 0.05);
  driverMount.scale.setScalar(0.95);
  group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
    driverMount.add(avatarObj.group);
  }
  ...
  return {
    ...
    runtimeRefs: {
      ...
      flames: nitroFlames,
      driverMount,
      paintMaterials: [bodyMat, neonMat],
      animate() {},
    },
    id: 'cyber',
...
export function buildChibiCuteRacer(primaryColor = 0xff7ebb, accentColor = 0xffe66d, avatarId = 'rabbit', showDriver = true, nitroBoost = false) {
  ...
  // Driver Mount Point
  const driverMount = new THREE.Group();
  driverMount.position.set(0, 0.78, -0.1);
  driverMount.scale.setScalar(1.05);
  group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
  ...
  return {
    ...
    runtimeRefs: {
      ...
      flames: nitroFlames,
      driverMount,
      paintMaterials: [bodyMat, accentMat],
      animate(time) {
...
export function buildFormula1RealRacer(primaryColor = 0xd90429, accentColor = 0xffb703, avatarId = 'fox', showDriver = true, nitroBoost = false) {
  ...
  const liveryMat = new THREE.MeshPhysicalMaterial({
    color: primaryColor, roughness: 0.2, metalness: 0.7, clearcoat: 1.0,
  });
  ...
  const yellowSponsorMat = new THREE.MeshBasicMaterial({ color: accentColor });
  ...
  // Driver Mount Point
  const driverMount = new THREE.Group();
  driverMount.position.set(0, 0.52, -0.12);
  driverMount.scale.setScalar(0.90);
  f1Group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
    driverMount.add(avatarObj.group);
  ...
  return {
    ...
    runtimeRefs: {
      ...
      flames: nitroFlames,
      driverMount,
      paintMaterials: [liveryMat, yellowSponsorMat],
      animate(time) { rainLight.visible = Math.sin(time * 12) > 0; },
    },
    id: 'f1',
...
export function buildQuantumHoverRacer(primaryColor = 0x00f0ff, accentColor = 0xa000ff, avatarId = 'raccoon', showDriver = true, nitroBoost = false) {
  ...
  const driverMount = new THREE.Group();
  driverMount.position.set(0, 0.50, 0.1);
  driverMount.scale.setScalar(0.92);
  group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
  ...
  return {
    ...
    runtimeRefs: {
      ...
      flames: nitroFlames,
      driverMount,
      paintMaterials: [crystalMat, quantumCoreMat],
      animate(time) { coreMesh.scale.setScalar(1 + 0.12 * Math.sin(time * 6)); },
    },
    id: 'quantum',
...
export function buildRuggedOffroadBeast(
  ...
  const driverMount = new THREE.Group();
  driverMount.position.set(0, 0.56, 0.0);
  driverMount.scale.setScalar(0.98);
  group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
  ...
  wheelPos.forEach(([wx, wy, wz]) => {
    ...
    for (let s = -0.18; s <= 0.18; s += 0.07) {
      createMesh(new THREE.TorusGeometry(0.075, 0.025, 6, 9), springMat, shockGroup, [0, s, 0], [Math.PI / 2, 0, 0]);
    }
  ...
  return {
    ...
    runtimeRefs: {
      ...
      flames: nitroFlames,
      driverMount,
      paintMaterials: [bodyMat, springMat, accentRimRing],
      animate() {},
    },
    id: 'offroad',
...
export function buildBreezeKart(
  ...
  // Driver Mount Point
  const driverMount = new THREE.Group();
  driverMount.position.set(0, 0.44, -0.10);
  driverMount.scale.setScalar(0.96);
  group.add(driverMount);
  let avatarObj = null;
  if (showDriver) {
    avatarObj = buildCartoonAvatar(avatarId);
