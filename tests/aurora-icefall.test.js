...
function runAuroraRace(difficulty) {
  const race = new RaceDirector(new Track(getTrackDef('aurora-icefall')), {
...
for (const difficulty of ['normal', 'hard']) {
  test(`Aurora Icefall ${difficulty} AI field completes the technical layout`, {
    timeout: 20_000,
  }, () => {
    const race = runAuroraRace(difficulty);
